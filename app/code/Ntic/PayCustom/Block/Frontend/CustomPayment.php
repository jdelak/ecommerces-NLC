...
class CustomPayment extends \Magento\Framework\View\Element\Template
{
    protected $_cart;
    ...
    public function getGrandTotal(){
        $result = $this->getQuote()->getGrandTotal();
        if($this->_typeOrder=="subscription"){
            $result = $this->getCurrentTotal();
        }
        return $result;
    ...
    public function getIban(){
        ...
        $query = "SELECT iban FROM sales_order WHERE customer_id = $customerId AND iban!=''";
        $result = $this->pdo->query($query);
        return $this->encryptor->decrypt($result->fetch(\PDO::FETCH_ASSOC)['iban']);
        //return $customerId;
    ...
    public function getConfigPaymentAbo($productId){
        $resource = $this->_objectManager->get('Magento\Framework\App\ResourceConnection');
        $connection = $resource->getConnection();
        $tableName = $resource->getTableName('ntic_config_payment_abo');
        $sql = "SELECT * FROM ".$tableName." WHERE to_type = '".$productId."'";
        $result = $connection->fetchAll($sql);
        return $result;
    }
    ...
    public function getCurrentQuote(){
        ...
        //var_dump($item->getProductId().', '.$item->getName().', '.$item->getSku().', '.$item->getPrice().', '.$item->getQty().', '.$cat->getName());
