...
//compatibility for new versions of scheduler
    ...
    function add_export_methods(scheduler){
        ...
        scheduler._get_export_size = function(format, orientation, zoom, dpi, header, footer) {
            ...
            var headerElement = scheduler._els["dhx_cal_header"][0];
            var dataElement   = scheduler._els["dhx_cal_data"][0];

            var dataX = dataElement ? dataElement.scrollWidth : 0;
            var dataY = dataElement ? dataElement.scrollHeight : 0;
            var headY = headerElement ? headerElement.scrollHeight : 0;
            ...
            if (format === "full") {
                ...
                cSize.x = Math.floor( (orientation === "landscape" ? sizes[format].x : sizes[format].y) * dpi );
            }
