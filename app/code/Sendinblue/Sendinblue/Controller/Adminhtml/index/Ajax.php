...
class Ajax extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (isset($post['manageSubsVal']) && $post['manageSubsVal'] == 'Update') {
                $this->ajaxSubscribeConfig(); 
            }

            if (isset($post['ord_track_btn']) && $post['ord_track_btn'] == 'Update') {
                $this->ajaxOrderStatus();
            }
            //SMTP settings enable or disable
            if (isset($post['smtp_post']) && $post['smtp_post'] == 'Update') {
            ...
            if (isset($post['sms_credit_post']) && $post['sms_credit_post'] == 'Update') {
                $this->ajaxSmsOperations();
            }

            //update order sms status.
            if (isset($post['order_setting_post']) && $post['order_setting_post'] == 'Update') {
                $this->ajaxSmsOperations();
            }

            //update shipped sms status.
            if (isset($post['shiping_setting_post']) && $post['shiping_setting_post'] == 'Update') {
                $this->ajaxSmsOperations();
            }

            //update campaign sms status.
            if (isset($post['campaign_setting_post']) && $post['campaign_setting_post'] == 'Update') {
                $this->ajaxSmsOperations();
            }

            //send test order sms.
            if(isset($post['order_send_post']) && ($post['order_send_post'] == 'Send')) {
            ...
            if(isset($post['shipped_send_post']) && ($post['shipped_send_post'] == 'Send')) {
                $sender = !empty($post['sender']) ? $post['sender'] : '';
                $message = !empty($post['message']) ? $post['message'] : '';
                $number = !empty($post['number']) ? $post['number'] : '';
                if (!empty($sender) && !empty($message) && !empty($number)) {
            ...
            if(isset($post['campaign_test_submit']) && ($post['campaign_test_submit'] == 'Send')) {
                $sender = !empty($post['sender']) ? $post['sender'] : '';
                $message = !empty($post['message']) ? $post['message'] : '';
                $number = !empty($post['number']) ? $post['number'] : '';
                if (!empty($sender) && !empty($message) && !empty($number)) {
            ...
            if (isset($post['order_import_post']) && $post['order_import_post'] == 'import_history') {
                $post = $this->getRequest()->getPostValue();
            ...
            //load contact list .
            if (isset($post['contact_data']) && $post['contact_data'] == 'contact_load') {
                $post = $this->getRequest()->getPostValue();
                $this->loadContact();
            }

            //Subscribe contact list .
            if (isset($post['contact_subs']) && $post['contact_subs'] == 'contact_subs') {
                $post = $this->getRequest()->getPostValue();
