...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            ...
            if (!$model->getId()) {
                $this->messageManager
                    ->addError(__('This slide no longer exists.'));
        ...
        $this->_coreRegistry->register('flexslider_slide', $model);

        $resultPage = $this->_initAction();

        $resultPage->getConfig()
            ->getTitle()->prepend(__('Flexslider - Slide Management'));
        $resultPage->getConfig()
            ->getTitle()->prepend(
                $model->getId() ? sprintf(__('Flexslider - Edit Slide "%s"'),
                $model->getTitle()) : __('Flexslider - New Slide')
            );
        return $resultPage;
    }
}
