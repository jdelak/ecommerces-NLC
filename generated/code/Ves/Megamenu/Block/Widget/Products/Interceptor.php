...
class Interceptor extends \Ves\Megamenu\Block\Widget\Products implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'checkProductIsNew');
        if (!$pluginInfo) {
            return parent::checkProductIsNew($_product);
        } else {
            return $this->___callPlugins('checkProductIsNew', func_get_args(), $pluginInfo);
        }
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'isNewProduct');
        if (!$pluginInfo) {
            return parent::isNewProduct($created_date, $num_days_new);
        } else {
            return $this->___callPlugins('isNewProduct', func_get_args(), $pluginInfo);
        }
    }
     ...
     * {@inheritdoc}
     */
    public function getVesProductPriceHtml(\Magento\Catalog\Model\Product $product, $priceType = null, $renderZone = 'item_list', array $arguments = array())
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'getVesProductPriceHtml');
        if (!$pluginInfo) {
            return parent::getVesProductPriceHtml($product, $priceType, $renderZone, $arguments);
        } else {
            return $this->___callPlugins('getVesProductPriceHtml', func_get_args(), $pluginInfo);
        }
    }
